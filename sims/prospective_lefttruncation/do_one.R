do_one <- function(n_train, n_test = 1000, estimator, dgp){
  ...
  if (estimator == "stackG_fine"){ # global stacking, all times grid
    ...
    fit <- survival::coxph(
      ...
      data = as.data.frame(cbind(time=train$Y, entry = train$W, event=train$Delta, train[1:dimension]))
    )
    ...
    if(ncol(pred) < length(benchmark_times)) {
      pred <- cbind(pred, matrix(pred[,ncol(pred)], nrow=nrow(pred), ncol=length(benchmark_times) - ncol(pred)))
    }
    est_df <- pred
    fits <- NA
    algos <- NA
